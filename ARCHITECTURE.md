...
| `sandbox_service.py` | Docker Sandbox lifecycle via the standalone `sbx` CLI: create, upload, exec, destroy — the older `docker sandbox` plugin this originally targeted has been deprecated and removed by Docker (see CLAUDE.md §6); this adapter now targets `sbx`, live-verified this session |
...
│   │   ├── github_service.py
│   │   ├── sandbox_service.py      sbx CLI adapter (Docker Sandboxes)
│   │   ├── pr_resolver.py
│   │   ├── job_repository.py
│   │   └── token_store.py
│   ├── core/
...
| Sandbox | `sbx` CLI adapter (Docker Sandboxes), plus `SANDBOX_MODE=local` Docker-free stopgap | **Complete** — both modes live-verified on this dev host as of a later session (see §8.1 correction below); `docker` mode's earlier WSL2 failure did not reproduce with `sbx`, the current CLI |
...
1. **Correction, later session: Docker Sandboxes work on this development host after all.** The original claim below (kept for the historical record, not because it's still true) was that a live `docker sandbox create ... shell ...` probe failed consistently with `starting LinuxKit VM: VM exited ungracefully during shutdown: context canceled`, traced to a WSL2 kernel update this machine had no admin rights to install. Two things have since changed: Docker Desktop's general engine now starts cleanly here (see CLAUDE.md §6), and Docker has replaced the `docker sandbox` CLI *plugin* entirely with a standalone `sbx` CLI for its "Docker Sandboxes" product — `sbx` is live-verified working on this exact machine (a full create → exec → remove cycle), so the sandbox stage no longer needs `SANDBOX_MODE=local` here specifically. `local` (§ "Sandbox boundary" above) remains a legitimate stopgap for hosts that genuinely cannot run Docker Sandboxes at all, just not this one.
   > *Original text, preserved:* Docker Sandboxes are confirmed broken on the current development host, not just unverified. A live `docker sandbox create ... shell ...` probe this session failed consistently with `starting LinuxKit VM: VM exited ungracefully during shutdown: context canceled`, traced to Docker Desktop's engine requiring a WSL2 kernel update that cannot be installed without admin rights on this machine. `SANDBOX_MODE=local` (§ "Sandbox boundary" above) exists specifically to unblock live verification on hosts like this one; it is not a substitute for fixing the real Docker Sandbox path where a working host is available.
